...
host_to_client_affine_matrix = generateAffineMatrix3DSelfLU(
    sample_data.host_matrix, sample_data.client_matrix
)
print("初期のアフィン行列の生成")
...
client_right_position = sample_data.client_matrix[0][:3, 3]
...
# x軸を揃えるための回転させる
host_to_client_affine_matrix = rotation(
    "z", client_world_host_hand_arrows_shift, host_to_client_affine_matrix
)


print("host_to_client_affine_matrix_z")
print(host_to_client_affine_matrix)
...
client_hands_positions = [
    sample_data.client_matrix[i][:3, 3] for i in range(len(sample_data.client_matrix))
]
host_hands_positions = [
    sample_data.host_matrix[i][:3, 3] for i in range(len(sample_data.host_matrix))
]
...
ani.save("client_world_rotate_y_base_swift_default.gif", writer="pillow")
